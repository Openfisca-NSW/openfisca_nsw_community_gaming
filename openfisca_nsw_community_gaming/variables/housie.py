...
class social_housie__game_meets_criteria(Variable):
    ...
    def formula(organisation, period, parameters):
        return (
            (organisation('gaming_activity_solely_for_social_purposes', period) * organisation('venue_is_licensed_premises', period) * (organisation('net_proceeds_returned_to_participants', period) * (organisation('condition_for_no_individual_prize', period)))))
...
class condition_for_no_individual_prize(Variable):
    ...
    def formula(organisation, period, parameters):
        return select(
            ...
            [(organisation('value_of_jackpot_prize', period) <= parameters(period).permitted_games.housie.max_value_of_jackpot_prize), organisation('total_prize_value_from_single_gaming_session', period) <= parameters(period).permitted_games.housie.max_value_of_prize_per_session.social_housie])
